...
export async function exportCapsule(raw: unknown, outputDirectory: string, capsuleId: string = randomUUID()): Promise<CapsuleManifest> {
  ...
  const artifacts = { "events.json": stringify(events), "replay.spec.ts": generatePlaywright(events), "report.html": reportFor(events, summary) };
...
export async function verifyCapsule(directory: string): Promise<VerificationResult> {
  ...
  } catch (error) { errors.push(error instanceof Error ? error.message : "Unable to read capsule."); }
